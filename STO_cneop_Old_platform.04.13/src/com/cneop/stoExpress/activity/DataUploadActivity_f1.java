...
public class DataUploadActivity_f1 extends CommonTitleActivity {
	...
	protected void initializeValues() {
		...
		orderOperService = new OrderOperService(DataUploadActivity_f1.this);
		siteProperties = GlobalParas.getGlobalParas().getSiteProperties();
		sourceList = new ArrayList<Map<String, Object>>();
		scanDataService = new ScanDataService(DataUploadActivity_f1.this);
		msgSendService = new MsgSendService(DataUploadActivity_f1.this);
		...
		// 初始化适配器
		dataUploadAdapter = new DataUploadAdapter(DataUploadActivity_f1.this, sourceList, R.layout.lv_data_upload_item, new int[] { R.id.chk_data_upload_item_ckSel, R.id.tv_data_upload_item_tvCount }, selMap);
	...
	private void uploadData() {
		if (sourceList.size() == 0) {
			PromptUtils.getInstance().showToast("没有需要上传的数据！", DataUploadActivity_f1.this);
			return;
		}
		boolean flag = true;
		for (Map<String, Object> map : sourceList) {
			...
			if (uploadView.getSelected()) {
				Message uploadMsg = UploadThread.getIntance(DataUploadActivity_f1.this).uploadHandler.obtainMessage();
				...
				UploadThread.getIntance(DataUploadActivity_f1.this).uploadHandler.sendMessage(uploadMsg);
				flag = false;
			}
		}
		if (flag) {
			PromptUtils.getInstance().showToast("请选择要上传的项！", DataUploadActivity_f1.this);
		} else {
			PromptUtils.getInstance().showToast("正在后台上传,请稍候！", DataUploadActivity_f1.this);
		}
	}
