...
public class RouteService extends BaseDao {
	...
	protected Object[] getInsertParams(Object object) {
		...
		Object[] objs = { route.getRouteId(), route.getRouteDesc(),
				route.getNextStationName(), route.getNextStationId(),
				route.getSecondStationName(), route.getSecondStatoinId(),
				route.getState(), route.getLasttime() };
		return objs;
	}
	...
	public Route getRoute(String routeNo) {
		if (!strUtil.isNullOrEmpty(routeNo)) {
			...
			routeModel = (Route) getSingleObj(words, selectArgs, routeModel);
			return routeModel;
		}
		return null;
	}
