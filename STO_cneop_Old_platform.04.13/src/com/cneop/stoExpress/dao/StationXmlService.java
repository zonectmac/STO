...
public class StationXmlService extends BaseDao {

	public StationXmlService(Context context) {
		super(context);
	...
	protected Object[] getInsertParams(Object object) {
		...
		Object[] objs = { station.getStationId(), station.getStationName(),
				station.getAttribute(), station.getState(),
				station.getLasttime() };
		return objs;
	}
	...
	public Station getStationModel(String stationId) {
		if (!strUtil.isNullOrEmpty(stationId)) {
			...
			stationModel = (Station) getSingleObj(words, selectArgs,
					stationModel);
			return stationModel;
		}
		return null;
